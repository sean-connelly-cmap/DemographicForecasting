...
projected_asfr <- base_year_pop_table %>%
  ...
  full_join(asfr_midpoint_cycle, by = c("age", "region_cc"))
...
projected_births_long <- projected_births |>
  select(region_cc, starts_with("birth")) |>
...
projected_births_long_by_sex <- projected_births_long %>%
  left_join(gender_ratios, by="region_cc") %>%
...
projected_births_surviving_0_4 <- projected_births_long_by_sex %>%
  left_join(mort_0_4, by="region_cc") %>%
...
early_deaths <- projected_births_long_by_sex %>%
  ...
  select(region_cc, sex, age, early_deaths)
...
pop_survival_rate_join <- base_year_pop_table %>%
  ...
  left_join(mort_midpoint_cycle, by=c('region_cc', 'age','sex')) %>%
  ungroup() |>
...
expected_pop <- pop_survival_rate_join %>%
  ...
  arrange(region_cc, sex, age_sorts_well) %>%
  ...
  left_join(projected_births_surviving_0_4, by = c("region_cc","sex","age")) %>%
...
gq_ratio_inst <- gq_ratios |>
  ...
  select(age, sex, region_cc, total_inst_gq_ratio)
...
civ_non_inst <- expected_pop_international_mig |>
  ...
  select(age, region_cc, sex, pop_with_intl_mig, num_in_lf, region, new_immigrants_region)
